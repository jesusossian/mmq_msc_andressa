...
def main():
	...
	#print(f"subsetp_rf = {len(subsetp_rf)}")
	#print(f"subsetr_rf = {len(subsetr_rf)}")
	...
	#print(f"opt = {round(rf_objval,2)}, time = {round(rf_rtime,2)}")

	#input("\n digite <enter> para continuar")

	#print(f"subsetp_fo = {len(subsetp_fo)}")
	#print(f"subsetr_fo = {len(subsetr_fo)}")
	...
	#print(f"opt = {round(fo_objval,2)}, time = {round(fo_rtime,2)}")

	#input("\n digite <enter> para continuar")

	arquivo = open(os.path.join(result_path,'clsr_std_rf_fo.txt'),'a')
	...
	objval, objbound, mgap, rtime, ncount, tmp = opt.clsr_std_math(N, PP, PR, FP, FR, HP, HR, D, R, SD, SR, C, yp_val, yr_val)
	...
	objval, objbound, mgap, rtime, ncount, tmp = opt.clsr_std_mip(N, PP, PR, FP, FR, HP, HR, D, R, SD, SR, C)
	...
	objval, rtime = opt.clsr_std_lp(N, PP, PR, FP, FR, HP, HR, D, R, SD, SR, C)
	
	arquivo = open(os.path.join(result_path,'clsr_std_lp.txt'),'a')
	arquivo.write(file_name+';'
	 	+str(round(objval,2))+';'
	 	+str(round(rtime,2))+';'
	 	+'\n')
